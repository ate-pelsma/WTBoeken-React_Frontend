...
export const User = ({ user, handleUserInactive }) => {
    ...
    return (
        <tr>
            <td><span className="pointer-hover" onClick={() => navigate(`/users/details/${id}`)}>{name}</span></td>
            <td><span className="pointer-hover" onClick={() => navigate(`/users/details/${id}`)}>{username}</span></td>
            <td>{role}</td>
            <td>
                <svg 
            ...
            <td className="align-middle">
                <svg 
                    ...
                    height="16" 
                    fill="currentColor" 
                    className="bi bi-trash-fill pointer-hover" 
                    viewBox="0 0 16 16" 
